...
module.exports = {
    ...
    async execute(client, interaction) {
        ...
        const message = await interaction.editReply({ content: "Please select which rule was broken by this user.", ephemeral: true, components: [reasonSelector] });
        const filter = i => {
            i.deferUpdate();
            return i.user.id == interaction.user.id;
        }
        const choise = await message.awaitMessageComponent({ filter, componentType: ComponentType.StringSelect, time: 60000}).catch(err => console.error(err));
        if (interaction.memberPermissions.has(PermissionFlagsBits.Administrator)) {
            ...
            const target = client.userReports.filter(report => report.reportedUserId == interaction.targetId).filter(report => report.userId == interaction.user.id)[0].targetUser;
            const member = await interaction.guild.members.fetch({ user: target.id, force: true });
            ...
            await channels.find(channel => channel.name == "blocked-messages").send({ content: `<@${interaction.guild.members.me.id}> has warned <@${target.id}>`, embeds: [embed] });
            await target.send(`You have been reported in our server for breaching the server rules or the Discord Community Guidelines.\n${keys[choise.values[0]]}\nYou can read the Community Guidelines on https://discord.com/guidelines.`);
            await interaction.editReply({ content: `You reported "${member.displayName}" for the following reason: "${choise.values.join(", ")}".`, ephemeral: true, components: [] });
            console.log(`> WARNED after REPORT: user ${member.displayName} ${target.id} because of ${choise.values.join(", ")}.`);
        } else {
            client.userReports.filter(report => report.reportedUserId == interaction.targetId).filter(report => report.userId == interaction.user.id).filter(report => report.reason == "unknown")[0].reason = choise.values.join(", ");
            const target = client.userReports.filter(report => report.reportedUserId == interaction.targetId).filter(report => report.userId == interaction.user.id)[0].targetUser;
            const member = await interaction.guild.members.fetch({ user: target.id, force: true });
            const embed = new EmbedBuilder()
                ...
                const channels = await interaction.guild.channels.fetch();
                await channels.find(channel => channel.name == "blocked-messages").send({ content: `<@${interaction.user.id}> has reported <@${target.id}>`, embeds: [embed] });
                await interaction.editReply({ content: `You reported "${member.displayName}" for the following reason: "${choise.values.join(", ")}". Your report will be reviewed soon.`, ephemeral: true, components: [] });
                console.log(`> REPORTED: user ${member.displayName} ${target.id} reported by ${interaction.user.username} because of ${choise.values.join(", ")}.`);
        }
    }
}