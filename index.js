...
const { Client, GatewayIntentBits, Collection, InteractionType, ChannelType, ActionRowBuilder, ButtonBuilder, ButtonStyle } = require("discord.js"); //jasper: removing MessageActionRow, MessageButton, ActivityType, ActivityFlags, ActivityPlatform, Partials (see 8)
const fs = require("fs");
const { Player } = require("discord-player");
const { token, clientId, guildId } = require("./config.json");
// DECLARATIONS
const client = new Client({ intents: [GatewayIntentBits.Guilds, GatewayIntentBits.GuildVoiceStates, GatewayIntentBits.GuildMessages, GatewayIntentBits.GuildMessageReactions, GatewayIntentBits.MessageContent] }) //jasper: removed partials --> partials: [Partials.Message, Partials.Channel, Partials.Reaction]
client.player = new Player(client);
client.messageReports = new Array();
client.userReports = new Array();
client.temporaryVoiceChannels = new Array();
client.gifSpamViolationTracker = new Array(); //jasper: renamed from gifTracker
...
client.on("messageCreate", async message => {
    if (message.content.includes(".gif") || message.content.includes("tenor.com") || message.content.includes("giphy.com") || message.content.includes("imgur.com")) {
        ...
        let gifMessageAuthor = message.author; //japser: renamed from gifAuthor
        let gifMessageChannel = message.channel; //jasper: renamed from gifChannel
        ...
        let gifMessagesCounter = 0; //jasper: renamed from amountOfMessages
        await gifMessages.forEach(() => gifMessagesCounter += 1);
        if (gifMessagesCounter >= 3) {
            const userGifSpamViolations = client.gifSpamViolationTracker.find(object => object.id == message.author.id); //jasper: renamed from gifTrackerObject
            if (!userGifSpamViolations) {
                ...
                const authorGuildMember = message.guild.members.cache.get(message.author.id); //jasper: renamed from member
                const nowDate = new Date(); //jasper: renamed from now
                const midnightDate = new Date(); //jasper: renamed from night
                midnightDate.setHours(24, 0, 0, 0);
                const milisecondsToMidnight = midnightDate.getTime() - nowDate.getTime();
                const timeoutLengths = [10*1000, 1*60*1000-15*1000, 5*60*1000-15*1000, 10*60*1000-15*1000, 60*60*1000-15*1000, milisecondsToMidnight, 24*60*60*1000]; //jasper: renamed from times
...
client.on("voiceStateUpdate", async function(oldMember, newMember) {
    await client.temporaryVoiceChannels.forEach(voiceChannel => { //jasper: renamed from channel
...
client.on("interactionCreate", async interaction => {
    ...
    if (!client.queue) return; //jasper: added
...
client.player.on("trackStart", async function(queue, track) {
    const controlButtons = new ActionRowBuilder().addComponents( //jasper: renamed form controls
    ...
    client.player.musicAnnounceMessage = await queue.metadata.interaction.followUp({ content: `Started playing "${track.title}".`, components: [controlButtons] }); //jasper: relocate and rename from client.followUpInteraction
});
...
schedule.scheduleJob("0 0 * * *", () => {
    ...
    let voiceChannels = client.guilds.cache.get(guildId).channels.cache.filter(channel => channel.type == ChannelType.GuildVoice); //jasper: renamed from filteredChannels
    voiceChannels.forEach(async (voiceChannel) => { //jasper: renamed from channel
        var messagesInVoiceChannel = await voiceChannel.messages.fetch(true); //jasper: renamed from fetchedMessages
